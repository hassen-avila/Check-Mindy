...
async function getData(){
    ...
    var id = 1
    arrayMeds.map(eventos => eventos.id = id++)
    console.log(arrayMeds)


    let arrayLowPrice = arrayMeds.filter(e =>e.precio < 400)
    arrayLowPrice.map(e=> e.categoria="-$400")
    let arrayMidPrice = arrayMeds.filter(e =>e.precio > 400 && e.precio < 1000)
    arrayMidPrice.map(e=>e.categoria="$400-$1000")
    let arrayHighPrice = arrayMeds.filter(e=> e.precio >= 1000)
